...
public class Resort implements Listener {
    public static Location surface;
    public File resort_top = new File(Stalker.getInstance().getDataFolder(), "structures/resort_top.schem");
    public File resort_bottom_1 = new File(Stalker.getInstance().getDataFolder(), "structures/resort_bottom_1.schem");
    public File resort_bottom_2 = new File(Stalker.getInstance().getDataFolder(), "structures/resort_bottom_2.schem");
    ...
    public Resort(Stalker plugin) {
    }

    public void spawnResort(Player player) throws IOException {
    ...
    private void loadSchematic(Player player, com.sk89q.worldedit.world.World bWorld) throws IOException {
        ...
        try (ClipboardReader reader = format.getReader(new FileInputStream(resort_top))) {
        ...
        try (ClipboardReader reader = format.getReader(new FileInputStream(resort_bottom_1))) {
            Clipboard clipboard = reader.read();
        ...
        try (ClipboardReader reader = format.getReader(new FileInputStream(resort_bottom_2))) {
            Clipboard clipboard = reader.read();
    ...
    private void addLoot() {
        AddLoot.addLootToSpecificContainer(surface, 0, -6, 4, common, uncommon, rare, epic);
        AddLoot.addLootToSpecificContainer(surface, 0, -6, 5, common, uncommon, rare, epic);
    }
}
